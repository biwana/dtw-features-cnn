...
def main(argv):
    if len(argv) < 8:
        print("Error, Syntax: {0} [train/test] [dataset] [conv dim] [conv len] [input len] [input depth] [input method] [fold]".format(argv[0]))
        exit()
    global conv_shape
    ...
    ns.load_settings_dtwfeatures(dataset, conv_dim, input_len, input_depth, input_method, fold)

    run_name = "dtw-features-fc1024-lr{0}-adam-{1}-{2}conv-{3}-{4}-{5}-fold{6}".format(ns.LEARNING_RATE, conv_dim, conv_shape, dataset, input_depth, input_method, fold)  # +"-"+nowtime
